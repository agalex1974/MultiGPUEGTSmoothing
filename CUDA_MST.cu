...
__global__
void init_Edges(wghEdge<intT> *input, intT size, intT *u, intT *v, float *w, intT *id) {
...
void merge_vertices(intT *successors, size_t size)
{
    ...
    if (pos < size) {
        ...
        while (goon && (i++ < 50)) {
            ...
            goon = __any_sync(__activemask(), ssuccessor != successor);
...
struct binop_tuple_minimum {
    typedef thrust::tuple<float, intT, intT> T; // (w, v, id)
    __host__ __device__
