...
GridMeshCuda::GridMeshCuda(const Mesh& mesh, int num_cards, float eps, int bucketCount):
        ...
        dreverse_indexes(num_cards, nullptr),
...
void GridMeshCuda::InitStructure(const float *points_x,const float *points_y,const float *points_z,const I3* indexes) {
...
#pragma omp parallel for num_threads(num_cards)
    for (int dev_id = 0; dev_id < num_cards; dev_id++) {
        ...
        cudaMalloc((void**)& dreverse_indexes[dev_id], (size_t)pointsCount * sizeof(int));
        thrust::sequence(thrust::device, dreverse_indexes[dev_id], dreverse_indexes[dev_id] + pointsCount, 0);
        thrust::sort_by_key(thrust::device, dtemp_indexes, dtemp_indexes + pointsCount,
                            dreverse_indexes[dev_id], thrust::less<int>());
...
#pragma omp barrier
    ...
    int batch_count = 60000000;
#pragma omp parallel for num_threads(num_cards)
    for (int dev_id = 0; dev_id < num_cards; dev_id++) {
        std::cout << trianglesCount << batch_count << std::endl;
        cudaSetDevice(dev_id);
        ...
        //std::cout << trianglesCount << batch_count << std::endl;
        uint32_t start = 0;
        while (start < trianglesCount) {
            uint32_t batchTriangleCount = trianglesCount - start >= batch_count ? batch_count : trianglesCount - start;
            if (dev_id == 0){
                std::cout << batchTriangleCount << " " << trianglesCount << std::endl;
            }
            cudaMalloc((void**)& dtriangleIndexes[dev_id], (size_t)batchTriangleCount * sizeof(I3));
            cudaMemcpy(dtriangleIndexes[dev_id], triangleIndexes + start, (size_t)batchTriangleCount * sizeof(I3), cudaMemcpyHostToDevice);
            neighborHelpers[dev_id].triangleIndexes = dtriangleIndexes[dev_id];

            uint32_t threads = 1024;
            uint32_t blocks = (int) ceil((1.0 * batchTriangleCount) / threads);
            reverseTriangleIndexes<<<blocks, threads>>>(dtriangleIndexes[dev_id], dreverse_indexes[dev_id], batchTriangleCount);
            cudaDeviceSynchronize();

            cudaMalloc((void **) &dtriangleIndexesVisited[dev_id], 3 * (size_t) batchTriangleCount * sizeof(uint32_t));
            cudaMemset(dtriangleIndexesVisited[dev_id], 0, 3 * (size_t) batchTriangleCount * sizeof(uint32_t));
...
GridMeshCuda::~GridMeshCuda(){
    for (int dev_id = 0; dev_id < num_cards; dev_id++){
        ...
        cudaFree(dreverse_indexes[dev_id]);
